...
def test_grdview_grid_file_with_region_subset(region):
    """Run grdview by passing in a grid filename, and cropping it to a
    region."""
...
def test_grdview_with_perspective(gridfile, xrgrid):
    """Run grdview by passing in a grid and setting a perspective viewpoint
...
def test_grdview_on_a_plane_with_colored_frontal_facade(gridfile, xrgrid):
    """Run grdview by passing in a grid and plotting it on a z-plane whose
    frontal facade is colored gray, while setting a 3D perspective
    viewpoint."""
    fig_ref, fig_test = Figure(), Figure()
...
def test_grdview_surface_plot_styled_with_contourpen(gridfile, xrgrid):
    """Run grdview by passing in a grid with styled contour lines plotted on
...
def test_grdview_surface_mesh_plot_styled_with_meshpen(gridfile, xrgrid):
    """Run grdview by passing in a grid with styled mesh lines plotted on top
    of a surface mesh plot."""
    fig_ref, fig_test = Figure(), Figure()
...
def test_grdview_on_a_plane_styled_with_facadepen(gridfile, xrgrid):
    """Run grdview by passing in a grid and plotting it on a z-plane with
...
def test_grdview_drapegrid_dataarray(gridfile, xrgrid):
    """Run grdview by passing in both a grid and drapegrid as an
...
def test_grdview_wrong_kind_of_drapegrid(xrgrid):
    """Run grdview using drapegrid input that is not an xarray.DataArray or
    file."""
    dataset = xrgrid.to_dataset()  # convert xarray.DataArray to xarray.Dataset
    assert data_kind(dataset) == "matrix"
